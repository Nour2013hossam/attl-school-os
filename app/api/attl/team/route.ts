...
export async function GET(){
 ...
 const allowed=[UserRole.ATTL_MEMBER,UserRole.TRACK_LEAD,UserRole.ADMIN,UserRole.SUPER_ADMIN];
 ...
 const [members,tracks,projects]=await Promise.all([
  ...
  prisma.project.findMany({where:{owner:{role:{in:[UserRole.ATTL_MEMBER,UserRole.TRACK_LEAD]}}},orderBy:{updatedAt:"desc"},take:20,select:{id:true,title:true,status:true,progress,owner:{select:{name:true}}}})
 ]);
