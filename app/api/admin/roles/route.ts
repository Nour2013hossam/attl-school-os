...
export async function GET(){
 ...
 if(!session?.user?.id || !(await hasAnyPermission(session.user.id,session.user.role,["roles.read","permissions.read"]))) return NextResponse.json({error:"Forbidden"},{status:403});
