...
export async function PATCH(req:Request,ctx:{params:Promise<{id:string;milestoneId:string}>}){
 ...
 if(!(p.ownerId===s.user.id||([UserRole.ADMIN,UserRole.SUPER_ADMIN] as UserRole[]).includes(s.user.role))||!(await hasPermission(s.user.id,s.user.role,"projects.manage")))return NextResponse.json({error:"Forbidden"},{status:403});
