...
type User = {
  id:string; name:string; email:string; role:string; isActive:boolean; attlMembershipActive:boolean; gradeLevel:string|null; className:string|null; schoolId:string|null; xp:number; level:number; customRole?:{id:string;name:string}|null;
};
    ...
    {filtered.map(user=><article key={user.id} className="rounded-[20px] border border-black/[.04] bg-white/60 p-4">
      <div className="grid gap-4 lg:grid-cols-[1.4fr_.7fr_.5fr_.7fr] lg:items-center">
       ...
       <div className="space-y-2">
        {permissionsReady && can("roles.assign") && <select value={user.role} disabled={busy===user.id} onChange={e=>update(user.id,{role:e.target.value})} className="h-10 w-full rounded-[13px] border border-black/5 bg-white px-3 text-[9px] outline-none">{roles.filter(role=>role!=="SUPER_ADMIN"||myRole==="SUPER_ADMIN").map(role=><option key={role}>{role}</option>)}</select>}
       ...
       <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-black/[.04] px-3 py-2 text-center text-[8px] font-semibold text-black/45">{user.xp.toLocaleString()} XP · L{user.level}</span><span className={user.isActive?"rounded-full bg-green-500/10 px-3 py-2 text-center text-[8px] font-semibold text-green-600":"rounded-full bg-black/[.04] px-3 py-2 text-center text-[8px] text-black/35"}>{user.isActive?"Active":"Disabled"}</span>{user.attlMembershipActive&&<span className="rounded-full bg-blue-500/10 px-3 py-2 text-center text-[8px] font-semibold text-blue-600">ATTL ACTIVE</span>}</div>
