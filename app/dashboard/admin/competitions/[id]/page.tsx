...
export default function AdminCompetitionEditPage() {
  ...
  useEffect(() => {
    if (!params.id) return;
    fetch("/api/competitions/" + params.id, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((data) => {
        if (!data?.competition) return;
        const value = data.competition as Competition;
        setItem(value);
        setForm({
          title: value.title,
          ...
          deadlineAt: value.deadlineAt?.slice(0, 16) ?? "",
  ...
  async function save() {
    ...
    setMessage(response.ok ? "Competition updated." : (data.error ?? "Could not update competition."));
    if (response.ok && data.competition) setItem(data.competition);
    setSaving(false);
  }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[32px] bg-black p-7 text-white md:p-9">
        <p className="text-[9px] uppercase tracking-[.2em] text-blue-300">Admin OS · Competitions</p>
        <h1 className="mt-3 text-3xl font-semibold tracking-[-.05em] md:text-5xl">Edit competition.</h1>
        <p className="mt-3 text-sm text-white/40">{item?.title ?? "Loading competition..."}</p>
      </section>

      <section className="rounded-[30px] border border-white/80 bg-white/60 p-6 backdrop-blur-2xl md:p-8">
        <div className="grid gap-4 md:grid-cols-2">
          <input value={form.title} onChange={(e) => setForm((x) => ({ ...x, title: e.target.value }))} placeholder="Competition title" className="h-11 rounded-[14px] bg-white/80 px-3 text-[10px] outline-none" />
          <input value={form.organizer} onChange={(e) => setForm((x) => ({ ...x, organizer: e.target.value }))} placeholder="Organizer" className="h-11 rounded-[14px] bg-white/80 px-3 text-[10px] outline-none" />
          ...
          <input type="datetime-local" value={form.deadlineAt} onChange={(e) => setForm((x) => ({ ...x, deadlineAt: e.target.value }))} className="h-11 rounded-[14px] bg-white/80 px-3 text-[10px] outline-none" />
          ...
          <input value={form.url} onChange={(e) => setForm((x) => ({ ...x, url: e.target.value }))} placeholder="https://example.com" className="h-11 rounded-[14px] bg-white/80 px-3 text-[10px] outline-none" />
        </div>
