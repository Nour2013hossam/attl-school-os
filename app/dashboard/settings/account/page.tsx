...
export default function SettingsAccountPage(){
 const [name,setName]=useState(""); const [bio,setBio]=useState(""); const [gradeLevel,setGradeLevel]=useState(""); const [className,setClassName]=useState(""); const [avatarUrl,setAvatarUrl]=useState(""); const [interests,setInterests]=useState(""); const [portfolioUrl,setPortfolioUrl]=useState(""); const [message,setMessage]=useState(""); const [saving,setSaving]=useState(false); const [links,setLinks]=useState("");

 useEffect(()=>{fetch("/api/profile",{cache:"no-store"}).then(r=>r.json()).then(data=>{const u=data.user;if(!u)return;setName(u.name??"");setBio(u.bio??"");setGradeLevel(u.gradeLevel??"");setClassName(u.className??"");setAvatarUrl(u.avatarUrl??"");setInterests((u.studentProfile?.interests??[]).join(", "));setPortfolioUrl(u.studentProfile?.portfolioUrl??"");setLinks((u.profileLinks??[]).map((l:{platform:string;url:string})=>l.platform+"|"+l.url).join("\n"));});},[]);

 async function save(e:FormEvent){e.preventDefault();setSaving(true);setMessage("");const res=await fetch("/api/profile",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name,bio,gradeLevel,className,avatarUrl,interests:interests.split(",").map(v=>v.trim()).filter(Boolean),portfolioUrl,links:links.split("\n").map(v=>v.trim()).filter(Boolean).map(v=>{const [platform,...urlParts]=v.split("|");return{platform:platform.trim(),url:urlParts.join("|").trim()}}).filter(v=>v.platform&&v.url)})});const data=await res.json();setMessage(res.ok?"Account updated.":(data.error??"Could not update account."));setSaving(false);}

 return <div className="space-y-6">
  <section className="rounded-[32px] bg-black p-7 text-white md:p-8"><p className="text-[9px] uppercase tracking-[.2em] text-white/30">Settings</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em] md:text-5xl">Account</h1><p className="mt-3 max-w-xl text-sm text-white/40">Update your personal profile. Grade and class are managed by school administration.</p></section>
  <section className="rounded-[30px] border border-white/80 bg-white/60 p-6 backdrop-blur-2xl md:p-8"><form onSubmit={save} className="grid gap-5 md:grid-cols-2">
    ...
    <div><label className="mb-2 block text-[9px] uppercase tracking-[.16em] text-black/30">Grade</label><input value={gradeLevel} readOnly disabled className="h-12 w-full cursor-not-allowed rounded-[16px] border border-black/5 bg-black/[.025] px-4 text-xs text-black/45 outline-none"/></div>
    <div><label className="mb-2 block text-[9px] uppercase tracking-[.16em] text-black/30">Class</label><input value={className} readOnly disabled className="h-12 w-full cursor-not-allowed rounded-[16px] border border-black/5 bg-black/[.025] px-4 text-xs text-black/45 outline-none"/></div>
    ...
    <div className="md:col-span-2"><label className="mb-2 block text-[9px] uppercase tracking-[.16em] text-black/30">Public accounts</label><textarea value={links} onChange={e=>setLinks(e.target.value)} rows={4} placeholder={"github|https://github.com/username\nlinkedin|https://linkedin.com/in/username\ninstagram|https://instagram.com/username"} className="w-full rounded-[16px] border border-black/5 bg-white/80 px-4 py-3 text-xs outline-none"/><p className="mt-2 text-[9px] text-black/30">One account per line: platform|URL</p></div>
