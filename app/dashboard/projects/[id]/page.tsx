...
export default function ProjectDetailPage({ params }: { params: Promise<{ id:string }> }) {
  ...
  async function load(){
    ...
    const [p,u,f]=await Promise.all([
      ...
      fetch("/api/uploads?entityType=project&entityId="+encodeURIComponent(id),{cache:"no-store"}).then(r=>r.ok?r.json():{files:[]}).catch(()=>({files:[]})),
    ]);
  ...
  async function uploadFile(event:React.ChangeEvent<HTMLInputElement>){
    ...
    const form=new FormData();form.set("entityType","project");form.set("entityId",id);form.set("file",file);
    const res=await fetch("/api/uploads",{method:"POST",body:form});const data=await res.json();
    setSaving(false);setMessage(res.ok?"File uploaded.":(data.error??"Could not upload file."));
    event.target.value="";
    if(res.ok)load();
  }
  ...
  async function deleteFile(fileId:string){
    setSaving(true);setMessage("");
    const res=await fetch("/api/uploads/"+fileId,{method:"DELETE"});const data=await res.json();
    setSaving(false);setMessage(res.ok?"File deleted.":(data.error??"Could not delete file."));
    if(res.ok)load();
  }
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
      ...
      <div className="space-y-4">
        {permissionsReady && can("projects.participate") && <div className="rounded-[28px] border border-white/80 bg-white/60 p-5 backdrop-blur-2xl md:p-7">
          <div className="flex items-center justify-between gap-3"><div><p className="text-[8px] uppercase tracking-[.18em] text-black/25">Files</p><h2 className="mt-1 text-xl font-semibold">Project files</h2><p className="mt-2 text-[9px] text-black/30">Upload project documents, designs, demos and other files up to 15 MB.</p></div><label className="cursor-pointer rounded-[13px] bg-black px-4 py-2.5 text-[9px] font-semibold text-white">Upload<input type="file" className="hidden" onChange={uploadFile} disabled={saving}/></label></div>
          <div className="mt-4 space-y-2">{files.map(file=><div key={file.id} className="flex items-center gap-3 rounded-[16px] bg-white/60 p-3"><div className="flex h-9 w-9 items-center justify-center rounded-[11px] bg-black text-[8px] font-semibold text-white">FILE</div><div className="min-w-0 flex-1"><p className="truncate text-[9px] font-semibold">{file.fileName}</p><p className="mt-1 text-[8px] text-black/30">{(file.sizeBytes/1024/1024).toFixed(1)} MB · {file.owner.name}</p></div><a href={"/api/uploads/"+file.id} target="_blank" rel="noreferrer" className="rounded-[10px] bg-blue-500/10 px-2.5 py-2 text-[8px] font-semibold text-blue-600">Open</a>{(file.owner.id===project.owner.id || (permissionsReady && can("projects.manage"))) && <button onClick={()=>deleteFile(file.id)} disabled={saving} className="rounded-[10px] bg-red-500/10 px-2.5 py-2 text-[8px] font-semibold text-red-600">Delete</button>}</div>)}{files.length===0&&<p className="rounded-[16px] bg-black/[.025] p-5 text-[9px] text-black/30">No files yet.</p>}</div>
        </div>}
        {permissionsReady && can("projects.tasks.manage") && <div className="rounded-[28px] border border-white/80 bg-white/60 p-5 backdrop-blur-2xl md:p-7"><p className="text-[8px] uppercase tracking-[.18em] text-black/25">New task</p><h2 className="mt-1 text-xl font-semibold">Add work</h2><form onSubmit={createTask} className="mt-5 space-y-3"><input required value={title} onChange={e=>setTitle(e.target.value)} placeholder="Task title" className="h-11 w-full rounded-[14px] border border-black/5 bg-white px-3 text-[10px] outline-none"/><textarea value={description} onChange={e=>setDescription(e.target.value)} rows={4} placeholder="Description" className="w-full rounded-[14px] border border-black/5 bg-white px-3 py-3 text-[10px] outline-none"/><select value={assigneeId} onChange={e=>setAssigneeId(e.target.value)} className="h-11 w-full rounded-[14px] border border-black/5 bg-white px-3 text-[10px] outline-none"><option value="">Unassigned</option>{manageableUsers.map(u=><option key={u.id} value={u.id}>{u.name}</option>)}</select><input type="datetime-local" value={dueAt} onChange={e=>setDueAt(e.target.value)} className="h-11 w-full rounded-[14px] border border-black/5 bg-white px-3 text-[10px] outline-none"/><button disabled={saving} className="w-full rounded-[14px] bg-black px-4 py-3 text-[9px] font-semibold text-white disabled:opacity-40">{saving?"Creating...":"Create task"}</button></form></div>}
